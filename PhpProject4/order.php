            ...
            <div class="text1">
                ...
                <table style="border:3px black solid;border-collapse:collapse;" align="center"  cellpadding="10" border='1' bgcolor="white"  ;>
                    ...
                    <?php
                        ...
                        $sql = "SELECT * FROM allorder WHERE `member_id` = '$member_id' AND `is_locked`= 0 ";
                            ...
                            foreach ($record as $key => $value) {
                                if($key==2){
                                    ...
                                    echo '<td>'
                                            ...
                                            .'<input type="hidden" name="product_id" value="'.$price_result[0][0].'">'
                                            . '<input type="submit" name="op"  value="-">'.' '.$value.' '.'<input type="submit" name="op"  value="+">'
                                            . '</form>'
                                        .'</td>';
                        ...
                        echo '<td colspan="2">總卡路里: '.$total_c.' 卡</td>';
                        mysqli_close($conn);
                        if($total_price>0){
                            echo '<tr>';
                                echo '<td colspan="5">';
                                    echo '<form method="post" action="check_out.php">';
                                        echo '<button class="big_button" style="text-align:center;font-family:Microsoft JhengHei;" type="submit" name="submit" value="結帳" >結帳</button>';
                                     echo '</form>';
                                  echo '</td>';
                            echo '</tr>';
                        }
